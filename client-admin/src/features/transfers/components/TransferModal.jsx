...
const AVATAR_COLORS = [
    'bg-blue-600', 'bg-violet-600', 'bg-rose-500', 'bg-teal-600',
    'bg-amber-500', 'bg-indigo-600', 'bg-green-600', 'bg-pink-500',
];
...
const inputClass = (error) =>
    `w-full px-3 py-2.5 text-sm border rounded-lg bg-white text-gray-700 placeholder-gray-300 focus:outline-none focus:ring-2 transition-all duration-150 ${error
    ...
    return (
        <div className="relative" ref={ref}>
            <button
                ...
                disabled={disabled}
                onClick={() => setOpen(p => !p)}
                className={`w-full flex items-center justify-between px-3 py-2.5 text-sm border border-gray-200 rounded-lg bg-white text-gray-700 focus:outline-none focus:ring-2 focus:ring-orange/30 focus:border-orange transition-all duration-150 ${disabled ? 'opacity-60 cursor-not-allowed' : 'hover:border-gray-300 cursor-pointer'
                    }`}
            >
                    ...
                    {options.map(opt => (
                        <li key={opt.value}>
                            <button
                                ...
                                className={`w-full text-left px-3 py-2 text-sm transition-colors duration-100 ${value === opt.value ? 'bg-orange/10 text-orange font-semibold' : 'text-gray-700 hover:bg-gray-50'
                                    }`}
                            >
                                {opt.label}
                            </button>
...
export const TransferModal = ({ accounts, accountsLoading, accountsError, onSubmit, loading }) => {
    ...
    const { register, handleSubmit, watch, formState: { errors } } = useForm({
        defaultValues: { amount: '', description: '', numberAccountDestination: '', destinationHolder: '' },
    });
        ...
        <div className="bg-white rounded-2xl border border-gray-100 shadow-sm p-6">
            ...
            {accountsLoading ? (
                ...
                <form onSubmit={handleSubmit(handleFormSubmit)} noValidate className="space-y-5">
                        ...
                        <div className="grid grid-cols-2 gap-2">
                            {[
                                ...
                                <button
                                    ...
                                    className={`py-2.5 px-4 text-sm font-semibold rounded-lg border transition-all duration-150 cursor-pointer flex items-center justify-center gap-1.5 ${transferType === tab.id
                    ...
                    {transferType === 'externa' ? (
                        <>
                            <Field label="Número de cuenta destino" error={errors.numberAccountDestination?.message}>
                                <input
                                    type="text"
                                    placeholder="Ej. MO00000001"
                                    {...register('numberAccountDestination', { required: 'Este campo es obligatorio' })}
                        ...
                        <>
                            {transferType === 'favorita' && selectedFavorite && (
                                <div className="flex items-center gap-2.5 bg-orange/5 border border-orange/20 rounded-xl px-3.5 py-2.5">
                                    <div className={`w-8 h-8 rounded-full flex items-center justify-center text-white text-xs font-bold shrink-0 ${avatarColor(selectedFavorite.alias)}`}>
                                        {getInitials(selectedFavorite.alias)}
                                    </div>
                                    <div className="flex-1 min-w-0">
                                        <p className="text-xs font-bold text-main-blue">{selectedFavorite.alias}</p>
                                        <p className="text-xs text-gray-400 truncate">{selectedFavorite.accountNumber}</p>
                                    </div>
                                ...
                                <div className="bg-orange/5 border border-orange/20 rounded-xl px-4 py-3">
                                    ...
                                    <div className="flex items-center gap-2">
                                        <span className="text-sm font-bold text-gray-600">
                                            {parseFloat(amount).toLocaleString('es-GT', { minimumFractionDigits: 2 })} {currency}
                                        </span>
                                        <span className="text-orange font-bold">→</span>
                                        <span className="text-lg font-extrabold text-orange">
                                            Q {(parseFloat(amount) * exchangeRate).toLocaleString('es-GT', { minimumFractionDigits: 2 })}
                                        </span>
                                    </div>
