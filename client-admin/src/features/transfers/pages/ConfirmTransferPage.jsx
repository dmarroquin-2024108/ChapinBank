...
const DetailRow = ({ label, value, accent = false }) => (
    <div className="flex justify-between">
        ...
        <span className={`text-xs font-semibold ${accent ? 'text-orange' : 'text-main-blue'}`}>{value}</span>
    </div>
);
...
const ResultCard = ({ action, result }) => {
    ...
    return (
        <div className={`rounded-2xl border p-6 text-center ${accepted ? 'border-orange/20 bg-orange/5' : 'border-main-blue/20 bg-main-blue/5'}`}>
            <div className={`w-14 h-14 rounded-2xl flex items-center justify-center mx-auto mb-4 ${accepted ? 'bg-orange/10' : 'bg-main-blue/10'}`}>
                {accepted
            ...
            <h2 className={`text-base font-extrabold mb-1 ${accepted ? 'text-orange' : 'text-main-blue'}`}>
            ...
            {result && (
                <div className="bg-white rounded-xl p-4 text-left space-y-2 mb-5 border border-gray-100">
                    ...
                    {result.nuevoBalanceDestino && (
                        <DetailRow label="Nuevo saldo" value={`Q ${parseFloat(result.nuevoBalanceDestino).toLocaleString('es-GT', { minimumFractionDigits: 2 })}`} accent />
                    )}
                    {result.reembolso && (
                        <DetailRow label="Reembolso al emisor" value={`Q ${parseFloat(result.reembolso).toLocaleString('es-GT', { minimumFractionDigits: 2 })}`} />
                    )}
                    {result.transfer?.status && (
                        <div className="flex justify-between">
                            <span className="text-xs text-gray-400">Estado</span>
                            <span className={`text-xs font-semibold ${accepted ? 'text-orange' : 'text-main-blue'}`}>
                                {result.transfer.status}
                            </span>
                        </div>
                    )}
...
const AutoConfirmScreen = ({ action }) => {
    ...
    return (
        <div className="bg-white rounded-2xl border border-gray-100 shadow-sm p-10 flex flex-col items-center gap-4 text-center">
            <div className={`w-14 h-14 rounded-2xl flex items-center justify-center ${accepted ? 'bg-orange/10' : 'bg-red-50'}`}>
                <Loader2 size={26} className={`animate-spin ${accepted ? 'text-orange' : 'text-red-400'}`} />
            </div>
            <div>
        ...
        const run = async () => {
            ...
            try {
                ...
                if (res.success) {
                    ...
                    setError(res.error ?? 'No se pudo procesar la transferencia. El token puede haber expirado.');
                }
            } catch {
            ...
            <div className="flex items-center gap-4 mb-8">
                ...
                <div>
                    ...
                    <p className="text-sm text-gray-400 mt-0.5">
                        {isAutoFlow ? 'Procesando tu respuesta…' : 'Acepta o rechaza una transferencia recibida'}
                    </p>
                </div>
            </div>
