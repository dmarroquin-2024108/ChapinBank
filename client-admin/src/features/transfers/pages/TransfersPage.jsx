    ...
    return (
        <div className="bg-white rounded-2xl border border-gray-100 shadow-sm p-6">
            ...
            <div className="bg-gray-50 rounded-xl p-4 space-y-2 mb-5">
                ...
                <DetailRow label="Monto" value={`${transfer.currency} ${parseFloat(transfer.amount).toLocaleString('es-GT', { minimumFractionDigits: 2 })}`} accent />
                {transfer.currency !== 'GTQ' && (
                    <DetailRow label="Equivalente GTQ" value={`Q ${parseFloat(transfer.amountInGTQ).toLocaleString('es-GT', { minimumFractionDigits: 2 })}`} />
                )}
                {transfer.currency !== 'GTQ' && (
                    <DetailRow label="Tasa de cambio" value={`1 ${transfer.currency} = Q ${transfer.exchangeRate}`} />
                )}
                <DetailRow label="Comisión" value={`Q ${parseFloat(transfer.commision).toLocaleString('es-GT', { minimumFractionDigits: 2 })}`} />
                <DetailRow label="Nuevo saldo origen" value={`Q ${parseFloat(transfer.nuevoBalanceOrigen).toLocaleString('es-GT', { minimumFractionDigits: 2 })}`} />
    ...
    return (
        <div className="max-w-2xl mx-auto px-4 py-6">
            <div className="flex items-center gap-4 mb-8">
                ...
                <div>
                    <h1 className="text-xl font-extrabold text-main-blue leading-tight">Realizar transferencia</h1>
                    <p className="text-sm text-gray-400 mt-0.5">Envía dinero a cualquier cuenta o a tus favoritos</p>
                </div>
            </div>
