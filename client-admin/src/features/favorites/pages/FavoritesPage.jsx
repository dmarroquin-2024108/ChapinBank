...
const getInitials = (alias) =>
    alias.split(' ').slice(0, 2).map((w) => w[0]).join('').toUpperCase();
        ...
        <div className='space-y-5'>
            ...
            <div className='grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-3'>
                ...
                <div className='rounded-2xl bg-[#0a4f3a] p-4 flex items-center justify-between'>
                    <div>
                        <p className='text-[10px] font-bold tracking-widest text-white/50 uppercase mb-1'>Ahorro</p>
                ...
                <div className='rounded-2xl bg-[#F28C00] p-4 flex items-center justify-between'>
                    <div>
                        <p className='text-[10px] font-bold tracking-widest text-white/50 uppercase mb-1'>Monetaria</p>
                            ...
                            <table className='w-full text-sm min-w-[720px]'>
                                <thead>
                                    <tr className='border-b border-gray-100'>
                                        ...
                                        <th className='text-left text-[11px] font-semibold text-gray-400 uppercase tracking-wider px-5 py-3'>No. Cuenta</th>
                                        <th className='text-left text-[11px] font-semibold text-gray-400 uppercase tracking-wider px-5 py-3'>Tipo</th>
                                        <th className='text-left text-[11px] font-semibold text-gray-400 uppercase tracking-wider px-5 py-3'>Agregado</th>
                                        <th className='text-left text-[11px] font-semibold text-gray-400 uppercase tracking-wider px-5 py-3'>Acciones</th>
                                    </tr>
                                </thead>
                                <tbody>
                                    {filtered.map((fav) => (
                                        <tr key={fav._id} className='border-b border-gray-50 hover:bg-gray-50/60 transition-colors'>
                                            ...
                                            <td className='px-5 py-3'>
                                                <span className={`text-xs font-bold px-2.5 py-1 rounded-full ${fav.accountType === 'AHORRO' ? 'bg-green-100 text-green-700' : 'bg-orange-100 text-orange-700'}`}>
                                                    {fav.accountType}
                                                </span>
                                            </td>
            ...
            />
            <DeleteFavoriteModal isOpen={!!deleteTarget} onClose={() => setDeleteTarget(null)} favorite={deleteTarget} />
            <QuickTransferModal isOpen={!!txTarget} onClose={() => setTxTarget(null)} favorite={txTarget} />
        </div>
    );
};