...
export const FavoriteModal = ({ isOpen, onClose, mode = 'add', initialData = {} }) => {
    ...
    useEffect(() => {
        if (isOpen) {
            if (mode === 'edit' && initialData) {
                setFormData({ accountNumber: initialData.accountNumber || '', alias: initialData.alias || '' });
            } else {
    ...
    const handleSubmit = async (e) => {
        ...
        if (result.success) {
            showSuccess(mode === 'add' ? 'Cuenta agregada a favoritos' : 'Alias actualizado correctamente');
            onClose();
        } else {
