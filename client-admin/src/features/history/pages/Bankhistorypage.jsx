  ...
  return (
    ...
    >
      ...
      <div className="flex-1 min-w-0">
        <p className="text-xs font-bold text-main-blue font-mono truncate">{account.accountNumber}</p>
        <p className="text-[11px] text-gray-400 truncate">{account.ownerName ?? '—'}</p>
      </div>
      <ChevronRight size={14} className={`shrink-0 transition-colors ${selected ? 'text-main-blue' : 'text-gray-300'}`} />
    </button>
  );
};
    ...
    <div className="px-3 sm:px-6 py-4 sm:py-6 space-y-6">
      ...
      <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4">
        <div className="flex items-center gap-4">
          ...
          <div>
            <h1 className="text-xl font-extrabold text-main-blue leading-tight">Historial del banco</h1>
            <p className="text-sm text-gray-400 mt-0.5">Consulta movimientos por cuenta o revisa la actividad global</p>
          </div>
        </div>
        <button
            ...
            {!selectedAccount ? (
              ...
              <>
                <div className="px-5 py-3.5 border-b border-gray-100 flex items-center justify-between">
                  <div>
                    <p className="text-sm font-bold text-main-blue font-mono">{selectedAccount.accountNumber}</p>
                    <p className="text-xs text-gray-400">{selectedAccount.ownerName} · Ultimos 5 movimientos</p>
                  </div>
                </div>
                {errors.adminFilter ? (
                  <div className="p-4"><ErrorState message={errors.adminFilter} /></div>
      ...
      <div className="grid grid-cols-1 xl:grid-cols-3 gap-6">
        <div className="xl:col-span-2 bg-white rounded-2xl border border-gray-100 shadow-sm overflow-hidden">
          <div className="px-5 py-4 border-b border-gray-100">
            ...
            {!loadings.bankHistory && bankHistory.length > 0 && (
              <p className="text-xs text-gray-400 mt-0.5">{bankHistory.length} movimiento{bankHistory.length !== 1 ? 's' : ''}</p>
            )}
          </div>
          {errors.bankHistory ? (
            <div className="p-4"><ErrorState message={errors.bankHistory} /></div>
        ...
        <div className="bg-white rounded-2xl border border-gray-100 shadow-sm overflow-hidden">
          ...
          {errors.accountsByMovements ? (
            <div className="p-4"><ErrorState message={errors.accountsByMovements} /></div>
          ) : loadings.accountsByMovements ? (
            <LoadingState />
          ) : accountsByMovements.length === 0 ? (
            <EmptyState message="Sin datos disponibles" />
          ) : (
            <div>{accountsByMovements.map((item, index) => <AccountMovementRow key={item.accountNumber} item={item} index={index} />)}</div>
          )}
        </div>
      </div>
    </div>
