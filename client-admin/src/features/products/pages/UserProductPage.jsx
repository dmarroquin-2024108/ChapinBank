...
export const UserProductPage = () => {
    ...
    const { ownedProductIds, buying, error: txError, getMyTransactions, buyProduct } = useTransactionStore();
    ...
    useEffect(() => { if (txError) showError(txError); }, [txError]);

    const handleBuyClick = useCallback((product) => setBuyTarget(product), []);
    const handleBuyConfirm = useCallback(async ({ productId, accountNumber }) => {
