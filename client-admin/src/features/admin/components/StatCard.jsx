...
export const StatCard = ({ title, value, subtitle, icon: Icon, color }) => (
  <div className={`rounded-2xl p-3 sm:p-5 flex flex-col gap-2 sm:gap-3 min-h-[120px] ${COLOR_MAP[color]}`}>
    <div className='flex items-start justify-between gap-2 sm:gap-3'>
      <p className='text-[9px] sm:text-xs font-semibold uppercase tracking-wide sm:tracking-widest opacity-80 leading-tight flex-1 min-w-0'>{title}</p>
      {Icon && (
